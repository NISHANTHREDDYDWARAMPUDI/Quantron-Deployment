report 50002 "Prod. Order Shortage List_B2B"
{
    DefaultLayout = RDLC;
    RDLCLayout = 'Reports\Layouts\ProdOrderShortageList.rdl';
    ...
    dataset
    {
        dataitem("Production Order"; "Production Order")
        {
            ...
            column(DateOfFirstPurchOrderCaption; DateOfFirstPurchOrderCaptionLbl)
            {
            }
            column(CountOfPurchOrderCaption; CountOfPurchOrderCaptionLbl)
            {
            }
            column(LeadTimeCaption; LeadTimeCaptionLbl)
            {
            }
            dataitem("Prod. Order Line"; "Prod. Order Line")
            {
                ...
                dataitem("Prod. Order Component"; "Prod. Order Component")
                {
                    ...
                    column(CompItemLeadTime; CompItem."Lead Time Calculation")
                    {
                    }
                    column(DateOfFirstPurchOrder; DateOfFirstPurchOrder_g)
                    {
                    }
                    column(CountOfPurchOrder; CountOfPurchOrder_g)
                    {
                    }
                    ...
                    begin
                        ...
                        PurchLine_l.Reset;
                        PurchLine_l.Setrange("Document Type", PurchLine_l."Document Type"::Order);
                        PurchLine_l.Setrange(Type, PurchLine_l.Type::Item);
                        PurchLine_l.Setrange("No.", "Prod. Order Component"."Item No.");
                        PurchLine_l.Setfilter("Qty. to Receive", '<>%1', 0);
                        //PurchLine_l.SetRange("Shortcut Dimension 1 Code", "Prod. Order Line"."Shortcut Dimension 1 Code");
                        //PurchLine_l.SetRange("Shortcut Dimension 2 Code", "Prod. Order Line"."Shortcut Dimension 2 Code");
                        //PurchLine_l.SetRange("Location Code", "Prod. Order Line"."Location Code");
                        //PurchLine_l.SetRange("Variant Code", "Prod. Order Line"."Variant Code");

                        If PurchLine_l.FindFirst() then begin
                            DateOfFirstPurchOrder_g := PurchLine_l."Expected Receipt Date";
                            CountOfPurchOrder_g := PurchLine_l.Count;
                        end else begin
                            DateOfFirstPurchOrder_g := 0D;
                            CountOfPurchOrder_g := 0;
                        end;
                        if not SkipZeroInventory then
    ...
    requestpage
    {
        ...
        layout
        {
            area(Content)
            {
                field(SkipZeroInventory; SkipZeroInventory)
                {
                    Caption = 'Include Zero Needed Qty';
                    ApplicationArea = all;
                }
            }
        }
    ...
    var
        ...
        CountOfPurchOrder_g: Integer;
        SkipZeroInventory: Boolean;
        DateOfFirstPurchOrder_g: Date;
        ...
        DateOfFirstPurchOrderCaptionLbl: Label 'First Purch Order Date';
        CountOfPurchOrderCaptionLbl: Label 'No. of Purchase Orders';
        LeadTimeCaptionLbl: Label 'Lead Time';
